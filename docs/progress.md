...
- Transaction state now uses a generic `onchainRecordId` field so escrow IDs are not stored under payment-intent naming, and result-card labels are centralized by mode.
