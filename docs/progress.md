...
- Combined test command passes (`npm run test:all` -> 11 Vitest + 6 Rust tests = 17 tests).
...
- Escrow lifecycle actions (`Release Escrow` and `Refund Escrow`) are fully integrated in UI with authorization guards and live transaction execution.
...
- Transaction state now uses a generic `onchainRecordId` field so escrow IDs are not stored under payment-intent naming; result/hero/error copy and confirmation state use transaction-neutral naming where the flow spans native, contract, and escrow modes.
...
- GitHub Actions CI/CD workflow is active and passing on `main`.
