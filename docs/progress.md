...
- Transaction state now uses a generic `onchainRecordId` field so escrow IDs are not stored under payment-intent naming, result-card labels/copy are mode-aware, and confirmation state uses transaction-neutral naming.
